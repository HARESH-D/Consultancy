...
from tensorflow.keras.layers import Dense, TimeDistributed, LSTM, Bidirectional, GRU

df1 = pd.read_csv(r"Data_set_1.csv", parse_dates=[1], encoding='utf-8')
df2 = pd.read_csv(r"Data_set_2.csv", parse_dates=[1], encoding='utf-8')
...
df_prod = df.loc[df['product'] == "S4402E0C31"]
df_semi_prod_month = df_prod.groupby(pd.Grouper(key='date', freq='D')).sum().reset_index()
# # print(len(df_prod_month))
# if len(df_semi_prod_month) >= 130:
df1 = df_semi_prod_month
...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons):
    ...
    # design network
    model = Sequential()
    model.add(LSTM(n_neurons, return_sequences=True, batch_input_shape=(n_batch, X.shape[1], X.shape[2])))
    model.add(Bidirectional(LSTM(n_neurons, stateful=True)))
    model.add(Dense(y.shape[1]))
    model.compile(loss='mean_squared_error', optimizer='adam')
    # fit network
    # for i in range(nb_epoch):
    # history = model.fit(X, y, epochs=1, batch_size=n_batch, verbose=2, shuffle=False)
    model.fit(X, y, epochs=70, batch_size=n_batch, verbose=2, shuffle=False)
        # print(history)
    # model.reset_states()
    return model
